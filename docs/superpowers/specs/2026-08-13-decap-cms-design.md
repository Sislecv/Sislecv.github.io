...
浏览器 /admin (Decap CMS SPA,静态文件,托管于博客)
  │  登录弹窗 → base_url/auth
  ▼
CF Worker OAuth 代理 (decap-proxy.sislecv2.workers.dev 自定义子域名)
...
backend:
  ...
  branch: main
  base_url: https://decap-proxy.sislecv2.workers.dev
  auth_endpoint: /auth
...
- `wrangler.toml`:name=decap-proxy,route=decap-proxy.sislecv2.workers.dev(custom_domain)
- secrets:`GITHUB_OAUTH_ID`、`GITHUB_OAUTH_SECRET`(用户注册 OAuth App 后提供)
- 处理 `/auth`、`/callback`,其他路径返回 Hello

## 部署步骤(用户操作,写入 README)

1. GitHub 注册 OAuth App:Homepage URL = `https://decap-proxy.sislecv2.workers.dev`,Callback URL = `https://decap-proxy.sislecv2.workers.dev/callback`
2. 提供 Client ID / Secret → 配置到 Worker secrets
3. `wrangler deploy` 部署 Worker 到 decap-proxy.sislecv2.workers.dev
